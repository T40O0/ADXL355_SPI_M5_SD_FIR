...
## Note
 ...
 - **FIR coefficients (500 Hz mode)**: this sketch ships with `FIR/min500.cf`, an 80-tap minimum-phase FIR (200 Hz passband / 250 Hz stopband at -120 dB) generated by [Telemetra-1](https://telemetra.jp/). Replace `FIR/min500.cf` with your own minimum-phase 80-tap design (declared as `double min500[80] = {...};`) to change the response.
 - **Serial debug output**: per-sample `Time(msec), x, y, z` is emitted on Serial when `#define ENABLE_SERIAL_SAMPLES 1` at the top of the sketch. Default is `0` (disabled). At 500 Hz the four `Serial.print` calls per 1 ms loop saturate the UART and add jitter to the sampling cycle, so keep it `0` for normal recording — enable only for low-rate verification.
 ...
 - **Timestamp resolution**: the `Time(msec)` column is stamped with `millis()` at FIFO-drain time, which has 1 ms resolution. At 500 Hz (true Δt = 2 ms) this means consecutive samples often share the same millisecond (Δt = 0) or fall on adjacent millisecond ticks (Δt = 1–6 ms), producing visible local jitter even though the **total time span** (`last − first ≈ 60000 ms`) per file remains correct. Use this metric — not the per-sample Δt — to verify recording integrity. The **per-file row count** is nominally `hz × 60` but can fluctuate by ±one batch (`hz × SDWriteTime` rows, i.e. ±7500 at 500 Hz / ±1500 at 100 Hz) at minute boundaries: TaskRead is paced by the ESP32 system tick while file rollover follows the M5 RTC (PCF8563), and the two crystals drift independently. When cumulative drift crosses one batch period, two adjacent files temporarily show one extra and one short batch; row timestamps within each file stay accurate.
